...
# find the (date, city) pairs with the largest errors 
# (2021-6-12, KANSAS CITY) is a large outlier!
errors %>% mutate(abs_error_lo_current_AM = abs(error_lo_current_AM)) %>%
  select(date, city, error_lo_current_AM, abs_error_lo_current_AM) %>%
  arrange(desc(abs_error_lo_current_AM)) %>%
  head(10)

errors %>% mutate(abs_error_hi_current_AM = abs(error_hi_current_AM)) %>%
  select(date, city, error_hi_current_AM, abs_error_hi_current_AM) %>%
  arrange(desc(abs_error_hi_current_AM)) %>%
  head(10)
...
cities <- read.csv("email_cities.csv") %>% filter(CITY != "DOVER")
mean_errors <- mean_errors %>% mutate(lat = cities$LAT,
                                      lon = cities$LON,
...
ggsave("scripts/mean_error_lo.png", plot = plot_mean_error_lo,
       width = unit(6, "in"), height = unit(4, "in"))

filter(mean_errors, !(city %in% not_cont), abs(mean_error_lo) > 1.7)
...
plot_mean_error_hi <- ggplot() + 
  ...
  geom_label(data = mean_errors %>% filter(!(city %in% not_cont), abs(mean_error_hi) > 1.1),
             aes(x = lon, y = lat, label = city), alpha = 0.5, size = 1.5) +
...
ggsave("scripts/mean_error_hi.png", plot = plot_mean_error_hi,
       width = unit(6, "in"), height = unit(4, "in"))

filter(mean_errors, !(city %in% not_cont), abs(mean_error_hi) > 1.1)
...
# more plots
ggplot(mean_errors %>% filter(!(city %in% not_cont)), aes(x = lon, y = mean_error_hi)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)

ggplot(mean_errors %>% filter(!(city %in% not_cont)), aes(x = mean_error_hi, y = lat)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)

ggplot(mean_errors %>% filter(!(city %in% not_cont)), aes(x = climate, y = mean_error_hi)) +
  geom_point()

ggplot(mean_errors %>% filter(!(city %in% not_cont)), aes(x = mean_error_hi, y = mean_error_lo)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)

ggplot(mean_errors %>% filter(!(city %in% not_cont)), aes(x = mean_error_hi, y = mean_error_lo, col = climate)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
